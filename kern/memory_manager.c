...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size) {
	...
	int no_pages = new_size / PAGE_SIZE;
	for (int i = 0; i < no_pages; i++) {
		int ret = pf_add_empty_env_page(e, virtual_address + (i * PAGE_SIZE),0);
		if (ret == E_NO_PAGE_FILE_SPACE) {
			cprintf("no page file space at line 778 in allocatemem");
		}
	}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 FrameStart = virtual_address;
	for (int j = 0; j < no_pages; j++) {
		pf_remove_env_page(e, FrameStart + (j * PAGE_SIZE));
	}
	//2.Free ONLY pages that are resident in the working set from the memory
	FrameStart = virtual_address;
		struct WorkingSetElement *element;
		LIST_FOREACH(element, &(e->ActiveList))
		{
			int unmapedframes = 0;
			if (element->virtual_address >= FrameStart &&
					element->virtual_address < FrameStart + (no_pages * PAGE_SIZE))
			{
				element->empty = 1;
				...
				int second_list_size = LIST_SIZE(&(e->SecondList));
				if (second_list_size != 0) {
					struct WorkingSetElement *headOfSecond = LIST_FIRST(
							&(e->SecondList));

					LIST_REMOVE(&(e->SecondList), headOfSecond);
					LIST_INSERT_TAIL(&(e->ActiveList), headOfSecond);
					pt_set_page_permissions(e, headOfSecond->virtual_address,
				...
				if (unmapedframes == 1024) {
					struct Frame_Info *page_frame = get_frame_info(e->env_page_directory, (void*) ptr_page_table,&ptr);
					page_frame->references = 0;
					free_frame(page_frame);
		...
		LIST_FOREACH(element,&(e->SecondList))
		{
							...
							element->virtual_address < FrameStart + (no_pages * PAGE_SIZE))
			{
				...
				if (unmapedframes == 1024) {
					struct Frame_Info *page_frame = get_frame_info(
							e->env_page_directory, (void*) ptr_page_table,
							&ptr);
					free_frame(page_frame);
					page_frame->references = 0;
					pd_clear_page_dir_entry(e, element->virtual_address);
		...
		uint32 calculate_required_frames(uint32* ptr_page_directory,
			...
			for (; current_virtual_address < (start_virtual_address + size);
				...
				get_page_table(ptr_page_directory,
						(void*) current_virtual_address, &ptr_page_table);
			...
			current_virtual_address = ROUNDDOWN(start_virtual_address,
					PAGE_SIZE);
			...
			for (; current_virtual_address < (start_virtual_address + size);
					...
					{
				uint32 *ptr_page_table;
				if (get_frame_info(ptr_page_directory,
						(void*) current_virtual_address, &ptr_page_table)
						== 0) {
					(number_of_pages)++;
				}
...
/// Dealing with environment working set
		...
		inline void env_page_ws_invalidate(struct Env* e,
				uint32 virtual_address) {
			int i = 0;
			for (; i < e->page_WS_max_size; i++) {
		...
		inline uint32 env_page_ws_is_entry_empty(struct Env* e,
				uint32 entry_index) {
		...
		void env_page_ws_print(struct Env *curenv) {
			...
			for (i = 0; i < (curenv->page_WS_max_size); i++) {
				...
				cprintf(", used= %d, modified= %d, buffered= %d", isUsed,
						isModified, isBuffered);
...
// Table Working Set =========================================================
		...
		inline void env_table_ws_invalidate(struct Env* e,
				uint32 virtual_address) {
			int i = 0;
			for (; i < __TWS_MAX_SIZE; i++) {
		...
		inline void env_table_ws_set_entry(struct Env* e, uint32 entry_index,
			...
			assert(virtual_address >= 0 && virtual_address < USER_TOP);
			e->__ptr_tws[entry_index].virtual_address = ROUNDDOWN(
					virtual_address, PAGE_SIZE*1024);
		...
		inline uint32 env_table_ws_get_virtual_address(struct Env* e,
				uint32 entry_index) {
			assert(entry_index >= 0 && entry_index < __TWS_MAX_SIZE);
			return ROUNDDOWN(e->__ptr_tws[entry_index].virtual_address,
					PAGE_SIZE*1024);
		}

		inline uint32 env_table_ws_is_entry_empty(struct Env* e,
				uint32 entry_index) {
...
/// Dealing with page and page table entry flags
		...
		inline uint32 pd_is_table_used(struct Env* ptr_env,
				uint32 virtual_address) {
			return ((ptr_env->env_page_directory[PDX(virtual_address)]
					& PERM_USED) == PERM_USED ? 1 : 0);
		}

		inline void pd_set_table_unused(struct Env* ptr_env,
				uint32 virtual_address) {
			ptr_env->env_page_directory[PDX(virtual_address)] &= (~PERM_USED);
			tlb_invalidate((void *) NULL, (void *) virtual_address);
		}

		inline void pd_clear_page_dir_entry(struct Env* ptr_env,
				uint32 virtual_address) {
		...
		extern int __pf_write_env_table(struct Env* ptr_env,
				uint32 virtual_address, uint32* tableKVirtualAddress);
		extern int __pf_read_env_table(struct Env* ptr_env,
				uint32 virtual_address, uint32* tableKVirtualAddress);
		...
		inline void pt_set_page_permissions(struct Env* ptr_env,
			...
			handling code");
				...
				__pf_write_env_table(ptr_env, virtual_address,
						(void*) ptr_temp_page);
			} else {
