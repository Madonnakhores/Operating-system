...
bool checkSecondList(struct Env * curenv, uint32 fault_va)
{
	bool isFound = 0;
	struct WorkingSetElement *secondChanceElement ;
	LIST_FOREACH(secondChanceElement, &(curenv->SecondList))
	{
		if(secondChanceElement->virtual_address == fault_va ) //Found in second chance list.
		{
			isFound=1;
			struct WorkingSetElement *tailOfActiveList = LIST_LAST(&(curenv->ActiveList));
			LIST_REMOVE(&(curenv->ActiveList), tailOfActiveList);
			LIST_REMOVE(&(curenv->SecondList), secondChanceElement);
			LIST_INSERT_HEAD(&(curenv->ActiveList), secondChanceElement);
			pt_set_page_permissions(curenv, secondChanceElement->virtual_address, PERM_PRESENT|PERM_WRITEABLE|PERM_USER, 0);
			LIST_INSERT_HEAD(&(curenv->SecondList), tailOfActiveList);
		    pt_set_page_permissions(curenv,tailOfActiveList->virtual_address,0,PERM_PRESENT|PERM_WRITEABLE|PERM_PRESENT);
		    return isFound;
		}
	}
	return isFound;
}
void allocateMap (struct Env * curenv, uint32 fault_va)
{
	struct Frame_Info *pointer_newframe = NULL;
	int return_allocate = allocate_frame(&pointer_newframe);
	int return_mapping = map_frame(curenv->env_page_directory,pointer_newframe,(void*)fault_va,PERM_WRITEABLE|PERM_USER);
}
struct WorkingSetElement* readingAndCheckStack(struct Env * curenv, uint32 fault_va)
{
	struct WorkingSetElement * element = LIST_LAST(&(curenv->PageWorkingSetList));
	element->virtual_address = fault_va;
	element->empty = 0;
	LIST_REMOVE(&(curenv->PageWorkingSetList), element);
	int readReturn = pf_read_env_page(curenv, (void*)fault_va);
	if(readReturn ==  E_PAGE_NOT_EXIST_IN_PF)
	{
		if(fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
		{
			...
			//cprintf("USTACKBOTTOM is %x\n" ,USTACKBOTTOM);
			//cprintf("USTACKTOP is %x\n" ,USTACKTOP);
			panic("This fault address doesn't exist in both page file and stack!");
		}
	}
	return element;
}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	//   DEBUG
	   ...
	   // if ((pt_get_page_permissions(curenv, fault_va) & (PERM_PRESENT | PERM_WRITEABLE)) == (PERM_PRESENT | PERM_WRITEABLE))
	    ...
	    char tmp[1024];
	    //////////////-
	int sizeOfActiveList = LIST_SIZE(&(curenv->ActiveList));
	int sizeOfSecondList = LIST_SIZE(&(curenv->SecondList));
	if(sizeOfActiveList < curenv->ActiveListSize)
	{
//		cprintf("test is here\n");
//		cprintf("sizeOfActiveList %d \n", sizeOfActiveList);
//		cprintf("curenv->ActiveListSize %d \n" , curenv->ActiveListSize);
		allocateMap(curenv, fault_va);
		struct WorkingSetElement * element= readingAndCheckStack(curenv, fault_va);
	...
	else if(sizeOfActiveList == curenv->ActiveListSize && sizeOfSecondList < curenv->SecondListSize)
	{
		bool isFound = checkSecondList(curenv, fault_va);
		if(!isFound) //NOT IN SECOND CHANCE LIST.
		{
			//cprintf("test 2 is here\n");
			allocateMap(curenv, fault_va);
			struct WorkingSetElement *tempElement = LIST_LAST(&(curenv->ActiveList)) ;
			LIST_REMOVE(&(curenv->ActiveList), tempElement);
			LIST_INSERT_HEAD(&(curenv->SecondList), tempElement);
			pt_set_page_permissions(curenv,LIST_FIRST(&(curenv->SecondList))->virtual_address, 0 ,PERM_PRESENT);
			struct WorkingSetElement * element = readingAndCheckStack(curenv, fault_va);
			LIST_INSERT_HEAD(&(curenv->ActiveList), element);
			pt_set_page_permissions(curenv, fault_va, PERM_PRESENT|PERM_USER|PERM_WRITEABLE, 0);
		}
	}
	else if(sizeOfActiveList == curenv->ActiveListSize && sizeOfSecondList == curenv->SecondListSize && curenv->SecondListSize !=0) //sizeOfSecondList
	{
		bool isFound = checkSecondList(curenv, fault_va);
		if(!isFound) //NOT IN SECOND CHANCE LIST.
		{
		    ...
		    if(page_permissions & PERM_MODIFIED) //Page Modified.
		    {
		    	uint32 *pointer = NULL;
		    	struct Frame_Info *frame = get_frame_info(curenv->env_page_directory,(void*)victim->virtual_address, &pointer);
		    	pf_update_env_page(curenv, (void*)victim->virtual_address, frame);
		    	LIST_REMOVE(&(curenv->SecondList), victim);
		    	unmap_frame(curenv->env_page_directory, (void*)victim->virtual_address);
		    }
		    else //Page Not Modified.
		    {
		    	LIST_REMOVE(&(curenv->SecondList), victim);
		    	unmap_frame(curenv->env_page_directory, (void*)victim->virtual_address);
		    }
		   // cprintf("test 3 is here\n");
		    allocateMap(curenv, fault_va);
	...
	else if(sizeOfActiveList == curenv->ActiveListSize && curenv->SecondListSize == 0)
	{
