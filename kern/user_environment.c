...
struct UserProgramInfo userPrograms[] =
		{ { "fos_helloWorld", "Created by FOS team, fos@nowhere.com",
				...
				{ "tpp1", "", PTR_START_OF(tst_placement_1) }, { "tpp2", "",
						PTR_START_OF(tst_placement_2) }, { "tpp3", "",
						PTR_START_OF(tst_placement_3) }, { "tia",
						"tests handling of invalid memory access", PTR_START_OF(
								tst_invalid_access) }, { "tpr1", "",
						PTR_START_OF(tst_page_replacement_LRU_Lists_1) }, {
						"tpr2", "", PTR_START_OF(
								tst_page_replacement_LRU_Lists_2) },
				...
				{ "tm4", "tests malloc (4): start address & allocated frames",
						PTR_START_OF(tst_malloc_4) },
				{ "tm5",
						"tests malloc (5): writing & reading values in allocated spaces",
						PTR_START_OF(tst_malloc_5) },
				{ "tm6",
						"tests malloc (6): check memory allocation and WS after accessing",
						PTR_START_OF(tst_malloc_6) },
				//BUDDY SYSTEM for SMALL SIZES ALLOCATIONS
				{ "tmb1", "", PTR_START_OF(tst_buddy_system_allocation_1) }, {
						"tfb1", "", PTR_START_OF(
								tst_buddy_system_deallocation_1) }, { "tfb2",
						"", PTR_START_OF(tst_buddy_system_deallocation_2) },

				//[2] PROGRAMS
				{ "tqsfh", "Quicksort with freeHeap", PTR_START_OF(
						tst_quicksort_freeHeap) }, { "fact",
						"Factorial Recursive", PTR_START_OF(fos_factorial) }, {
						"fib", "Fibonacci Recursive", PTR_START_OF(
								fos_fibonacci) }, { "tqs",
						"Quicksort that cause memory leakage", PTR_START_OF(
								quicksort_heap) }, { "qs",
				...
				//[3] BONUSES
				{ "tfs", "Tests freeing the unused stack pages", PTR_START_OF(
						tst_freeing_stack) }, { "ef_fib", "", PTR_START_OF(
						ef_fos_fibonacci) }, { "ef_fact", "", PTR_START_OF(
						ef_fos_factorial) }, { "ef_fos_add", "", PTR_START_OF(
						ef_fos_add) },
				{ "tef1", "", PTR_START_OF(tst_envfree1) }, { "ef_ms1", "",
						PTR_START_OF(ef_mergesort_noleakage) }, { "ef_ms2", "",
						PTR_START_OF(ef_mergesort_leakage) }, { "tef2", "",
						PTR_START_OF(tst_envfree2) },
				...
				{ "qs1", "Quicksort v.1", PTR_START_OF(quicksort1) }, { "qs2",
						"Quicksort v.2", PTR_START_OF(quicksort2) }, { "qs3",
						"Quicksort v.3", PTR_START_OF(quicksort3) }, { "qs4",
						"Quicksort v.4", PTR_START_OF(quicksort4) }, { "qs5",
						"Quicksort v.5", PTR_START_OF(quicksort5) },
...
void ShareWSAtUserSpace(struct Env* e) {
	...
	for (sva = (uint32) (e->ptr_pageWorkingSet);
			sva < ((uint32) (e->ptr_pageWorkingSet) + nBytes);
			sva += PAGE_SIZE, dva += PAGE_SIZE) {
		...
		if (get_page_table(e->env_page_directory, (void*) dva,
				&ptr_page_table) == TABLE_NOT_EXIST) {
			ptr_page_table = create_page_table(e->env_page_directory,
					(uint32) dva);
		}
		ptr_page_table[PTX(dva)] = CONSTRUCT_ENTRY(pa,
...
void env_free(struct Env *e){
	...
	for(int i=0;i<PDX(USER_TOP);i++){
		...
		struct Frame_Info*pageframe=to_frame_info(e->env_page_directory[i]);
		free_frame(pageframe);
		}
	}
 uint32 pa = e->env_cr3;
	struct Frame_Info*pageframe=to_frame_info(pa);
	free_frame(pageframe);
